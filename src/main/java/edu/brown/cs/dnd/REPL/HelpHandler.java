      ...
      switch (args[1]) {
        ...
        case "s": {
          ...
          s += "- search <table> | <flags>: searches with a set of operators " +
              "in a table (e.g. search spells | class: bard, level: 2, or " +
              "search monsters | type: humanoid, ac: >= 18, int: >= 14)\n";
          s += "- Tip: you can also search with just “s” for short!";
          return toResult(s);
        }
        case "roll": {
          String s = "- roll <dice>: randomly rolls the result (e.g. roll 2d4" +
              " will roll two four-sided dice and sum the results together)\n";
        ...
        case "generate-npc": {
          ...
          s += "generate-npc <flags>: allows finer control over the " +
              "randomization. Separate flags by a space (e.g. generate-npc " +
        ...
        case "generate-dungeon":
          return toResult("generate-dungeon: <width> <height> <small, medium " +
              "or large>");
        case "help": {
